...
public class RoomsActivity extends ZorbaActivity
    ...
    private ListPopupWindow prepareHomeMenu() {
        ...
        popupWindow.setAdapter(textAdapter);
        popupWindow.setAnchorView(findViewById(R.id.homeButton));
        popupWindow
                .setWidth(CommonUtils.measureContentWidth(popupWindow.getListView(), textAdapter) + 60);
        ...
        try {
            ...
            if (!isNewRoom) {
                ...
                if (this.roomDataList.size() == 0) {
                    ...
                    findViewById(R.id.rgbPanel).setVisibility(View.GONE);
                    findViewById(R.id.emptydevicepanel).setVisibility(View.VISIBLE);
    ...
    private void controlDevice(final ImageTextButton paramImageTextButton, final String devtype, final int devid) {
        ...
        try {
            ...
            if (i != -1) {
                localObject.setProgress(i * 10);
            }
            localObject.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                int prevvalue = -1;
    ...
    private void populatePageForSelectedRoom() {
        ...
        if (isInitial) {
            ...
            findViewById(R.id.rootView).setBackgroundColor(Color.parseColor("#ff403c3a"));// 403c3a
            findViewById(R.id.scrollView).setBackgroundColor(Color.parseColor("#ff1e1e1e"));
            updateWithRealtime();
            relayout();
        }
    ...
    private void roomChanged(boolean isInitial, TextView paramTextView, int paramInt, boolean isFromSelection) {
        if (!isInitial && (this.selectedRoom != null)
                && (this.selectedRoom.getName().equals(this.roomDataList.get(paramInt).getName()))) {
            if (isFromSelection)
                return;
        }
        if (roomDataList.size() == 0)
            return;
        this.selectedRoom = this.roomDataList.get(paramInt);
        ...
        if (selectedRoom.isRGBType()) {
            ...
            findViewById(R.id.rgbPanel).setVisibility(View.GONE);
            findViewById(R.id.emptydevicepanel).setVisibility(View.GONE);
    ...
    public void relayout() {
        findViewById(R.id.overlay).post(new Runnable() {
            public void run() {
